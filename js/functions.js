...
function create_filter_element (data) {
  ...
  li.textContent = data.textContent; // textContent är Key på den här sit och är inte reserverad Ord
...
// CODE according to specifications
function create_programme(objctt) {
  let parent_new_program = document.createElement("div"); // continer för program med bid 
  let child_new_program = document.createElement("li"); // box för texten info in på continer
  let child_sun = document.createElement("li"); // box text in på bilden under m,ed sun text
  let granddad = document.querySelector("#programmes ul");
  let show_more = document.createElement("li");

  let info = { //den samlar all info som syns under program bild
    uni: UNIVERSITIES[objctt.universityID].name, // UNIVERSITIES arrayen använder parameterns Key universityID som index numer , name är key för objektet i UNIVERSITIES
    city: CITIES[UNIVERSITIES[objctt.universityID].cityID].name,
    sun_index: CITIES[UNIVERSITIES[objctt.universityID].cityID].sun,
    country: COUNTRIES[CITIES[UNIVERSITIES[objctt.universityID].cityID].countryID].name,
    level: LEVELS[objctt.levelID - 1].name, // måsta göra -1 för att array elementet börjar med id =1  
    subject: SUBJECTS[objctt.subjectID].name,
    language: LANGUAGES[objctt.languageID].name,
 ...
 */
  ...
  parent_new_program.appendChild(child_sun);
   ...
   // objctt måsta vara ett objekt från PROGRAMMES arrayen 
   // som en varibel som har ett objekt som en värde 
  child_new_program.innerHTML = `<h1>${objctt.name}</h1> 
    <p>${info.uni}</p> 
    <p>${info.city}, ${info.country}</p> 
    <p>${info.level}, ${info.subject}, ${info.language}</p>`
    child_sun.innerHTML = `<p>${info.city}, sun-index: ${info.sun_index} </p>`
...
function update_programmes () {
  ...
  let empty_string = document.querySelector ("#programes p");
